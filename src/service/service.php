...
class Service
{
     ...
     * @return array
     */
    public function getMetaData(string $routeName)
    {
        return $this->app['db']->fetchAssoc("SELECT title, description, keywords FROM pages WHERE routeName = ?", [(string)$routeName]);
    }
}